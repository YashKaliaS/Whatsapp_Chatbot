...
async def reply(request: Request, db: Session = Depends(get_db)):
    ...
    # Check if the message has a media URL (indicating it's a voice note)
    if 'MediaUrl0' in form_data:
        # It's a voice note, process as audio
        ...
        with open(mp3_file_path, "rb") as audio_file:
            ...
            print(f"Transcribed the voice note to the following text: {whisper_response.get('text')}. Now it's being sent to ChatGPT API to reply…")
        ...
        # It's a text message, process it directly
        print("Processing text message...")
        text_message = form_data['Body']
        print(f"Received text message: {text_message}")

        # Call the OpenAI API to generate a response with ChatGPT
    ...
    try:
        conversation = Conversation(
            ...
            message=whisper_response.get("text") if 'MediaUrl0' in form_data else text_message,
